...
#define NOOFPRED 5	//no of predictors
#define CORRECT 1	//correct prediction
#define WRONG 0		//wrong prediction
#define MAXWEIGHT 5	//maximum weight assigned to a predictor-saturation no

int weight[NOOFPRED];			//weightage for each predictor
static struct bpred_t *mypred[NOOFPRED];
...
{
	int i;
	for(i=0;i<NOOFPRED;i++)
	{
		weight[i]=0;
	}
}

void satcount(int predno,int status)	
//saturation counter for prediction .status-correct or incorrect..predno-no fo the predictor
{
	if(status==CORRECT)
	{
		if(weight[predno]+1<=MAXWEIGHT)
		{
			weight[predno]++;
		}
		else
		{
			weight[predno]=MAXWEIGHT;
		}
	}
...
/* branch predictor */
//static struct bpred_t *pred;			//THIS COMMENT IS MY WORK
...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
 ...
 //creating the predictors
	...
	mypred[0]=bpred_create(BPredTaken, 0, 0, 0, 0, 0, 0, 0, 0, 0);
    
        /* static predictor, taken */
      	mypred[1] = bpred_create(BPredNotTaken, 0, 0, 0, 0, 0, 0, 0, 0, 0);
		...
		fatal("bad btb config (<num_sets> <associativity>)");
      	...
      	mypred[2] = bpred_create(BPred2bit,
	...
	//2 level predictor
		...
		fatal("bad btb config (<num_sets> <associativity>)");
      	...
      	mypred[3] = bpred_create(BPred2Level,
			  /* bimod table size */0,
		...
		fatal("bad btb config (<num_sets> <associativity>)");
      	...
      	mypred[4] = bpred_create(BPredComb,
			  /* bimod table size */bimod_config[0],
...
sim_main()
{
	...
	//	MY VARIABLE ENDS
  	...
  	int j,k=0,l=0;	//this is MY VARIABLE
			int flag=0;
	  		md_addr_t pred_PC[NOOFPRED];
	  		md_addr_t PCvals[NOOFPRED];//THIS IS MY VARIABLE--predicted values votes
	  		int val[NOOFPRED];		//predicted values votes
	  		struct bpred_update_t update_rec[NOOFPRED];
	  		int max;
	  		md_addr_t maxPC;
	...
	default:
	  panic("attempted to execute a bogus opcode");
      		...
      		if (MD_OP_FLAGS(op) & F_CTRL)
		{
				...
				if (mypred[i])
	    			{
	    				//printf("\n pred_PC if %d",i);
	      				...
	      				pred_PC[i] = bpred_lookup(mypred[i],
				     	/* branch addr */regs.regs_PC,
				     	/* target */target_PC,
				     	/* inst opcode */op,
				     	/* call? */MD_IS_CALL(op),
				     	/* return? */MD_IS_RETURN(op),
				     	/* stash an update ptr */&update_rec[i],
	      		...
	      		/* valid address returned from branch predictor? */
	      				...
	      				if (!pred_PC[i])
					{
		  			/* no predicted taken target, attempt not taken target */
		  				pred_PC[i] = regs.regs_PC + sizeof(md_inst_t);
					}

	      				bpred_update(mypred[i],
			   			/* branch addr */regs.regs_PC,
			   			/* resolved branch target */regs.regs_NPC,
			   			/* taken? */regs.regs_NPC != (regs.regs_PC +
							 sizeof(md_inst_t)),
			   			/* pred taken? */pred_PC[i] != (regs.regs_PC +
							sizeof(md_inst_t)),
			   			/* correct pred? */pred_PC[i] == regs.regs_NPC,
			   			/* opcode */op,
			   			/* predictor update pointer */&update_rec[i]);
	    			}
	    
				//logic for calculating the prediction
			...
			maxPC=PCvals[0];
			max=val[0];
			//printf("\n maxPC and max assignment");
			for(j=1;j<NOOFPRED;j++)
			{
				if(max<val[j])
				{
					maxPC=PCvals[j];
			...
			//printf("\n correctpreds increament");
			if(maxPC!=regs.regs_NPC)
			{
				misses++;//incorrectpreds++;
			}
			//printf("\n sat count");
			for(j=0;j<NOOFPRED;j++)
			{
				if(pred_PC[j]==regs.regs_NPC)
				{
					satcount(j,CORRECT);
				}
				else
...
{
//	printf("\n\n\t\tMY OUTPUT");
  //  printf("\n\n\t\tTHE NO OF MISSES IS %d",incorrectpreds);
    //printf("\n\n\t\tTHE NO OF WRONG PREDICTIONS IS %d\n\n\n",sim_num_branches-incorrectpreds);
	...
	/*if(sim_num_insn==max_insts)
	{
		printf("\n\n\t\tMY OUTPUT");
    printf("\n\n\t\tTHE NO OF CORRECT PREDICTIONS IS %d",incorrectpreds);
    printf("\n\n\t\tTHE NO OF WRONG PREDICTIONS IS %d\n\n\n",sim_num_branches-incorrectpreds);
	}*/
	//if(syscode == SS_SYS_exit)
	//{
	//	printf("\n\n\t\tMY OUTPUT");
  //printf("\n\n\t\tTHE NO OF MISSES IS %d",misses);
  //	}
      ...
      if (max_insts && sim_num_insn >= max_insts)
      {
  		...
  		fflush(stdout);  	
      	//printf("\n\n\t\tMY OUTPUT");
  //  printf("\n\n\t\tTHE NO OF CORRECT PREDICTIONS IS %d",correctpreds);
    //printf("\n\n\t\tTHE NO OF WRONG PREDICTIONS IS %d\n\n\n",sim_num_branches-correctpreds);
	return;
	}
    }
  //  printf("\n\n\t\tMY OUTPUT");
