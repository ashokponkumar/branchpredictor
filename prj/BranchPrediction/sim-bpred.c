...
#define MAX_NO_ENTRIES 10000
#define MAX_NO_REGS 32


int no_good_pred,no_miss_pred,no_misses,no_hits,no_gp,no_mp,no_new,no_old;

/********* STRUCTS AND FUNCTIONS FOR LIST OF (ADDR , PRED) ****/
typedef struct list_node
{
	md_addr_t addr;// addr of instr which previously written the register
	int pred;// 0-not taken . 1- taken.
	md_addr_t pred_addr;
...
void insert(node * p,md_addr_t addr,int pred,md_addr_t pred_addr)
{
	node *q;
	node *t;
	...
	t=(node *)malloc(sizeof(node));
        ...
        t->pred=pred;
        t->next=NULL;
        t->pred_addr=pred_addr;				 
	q->next=t;
}

md_addr_t get_pred(node * p,md_addr_t addr)
{
	p=p->next;
...
// context based prediction table

struct prediction_table
{
	list p;
}ptable[MAX_NO_ENTRIES];

// tournament predictor

int tpred[MAX_NO_ENTRIES];

void init_table()
{
	int i;
	for(i=0;i<MAX_NO_ENTRIES;++i)
	{
		ptable[i].p=makelist();
		tpred[i]=2;
	}
...
int is_newpred_acceptable(int e)
{
	if(tpred[e] >= 2)
		return 1;
	else
...
{
	return (addr%MAX_NO_ENTRIES);
}

/******* STRUCTS AND FUN FOR TRACKING THE WRITER OF EACH OF REGS *****/

#define INT_REGS 0
#define FLOAT_REGS 1

int tracker[2][MAX_NO_REGS];

md_addr_t get_writer(int type,int reg_no)
{
	return tracker[type][reg_no];
}
void set_writer(int type,int reg_no,md_addr_t mem_addr)
{
	tracker[type][reg_no]=mem_addr;
}
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_counter(sdb, "sim_num_refs",
		   ...
		   &sim_num_refs, 0, NULL);
  stat_reg_int(sdb,"no_good_pred",
		  " No of correct predictions in the new predictor",
		  &no_good_pred,0, NULL);
  stat_reg_int(sdb,"no_gp",
		  " No of correct predictions in the old predictor",
		  &no_gp,0,NULL);
  stat_reg_int(sdb,"no_hits",
		  " No of hits in new predictor",
		  &no_hits,0, NULL);
  stat_reg_int(sdb,"no_miss_pred",
		  "No of mis predictions in new predictor",
		  &no_miss_pred, 0, NULL);
  stat_reg_int(sdb,"no_mp",
		  "No of mis predictions in old predictor",
		  &no_mp,0,NULL);
  stat_reg_int(sdb,"no_misses",
		  "Number of misses in new predictor",
		  &no_misses,0, NULL);
  stat_reg_int(sdb,"no_new",
		  "Number of times new predictor is used",
		  &no_new,0,NULL);
  stat_reg_int(sdb,"no_old",
                   "Number of times old predictor is used",
                    &no_old,0,NULL);
  ...
  stat_reg_formula(sdb, "sim_inst_rate",
		   "simulation veri thanam speed (in insts/sec)",
...
#include "machine.def"
      ...
      if (MD_OP_FLAGS(op) & F_MEM)
	{
      ...
      {
	      set_writer(FLOAT_REGS,RA,regs.regs_PC);      
      }

      if(MD_OP_FLAGS(op) & F_ICOMP)
      {
	      set_writer(INT_REGS,RA,regs.regs_PC);	      
      }
      ...
      if(MD_OP_FLAGS(op) & F_STORE)
      {
	      ...
	      //
	      //	      
  	      // to do depending on whether the store is on int or float reg.	
	      // 
	      // 
      }
	  ...
	  // RA is the register depending on which the jump must take place.
	  // so this is the register for which the recent writer must be checked.
	  // 
	  //default to allow the prediction
	  //
	  // does the context based prediction on the branch instruction
	  
	  pred_PC2=get_pred( ptable[ hash( regs.regs_PC ) ].p,get_writer(INT_REGS,RA));	  
...
/*	  else
	  ...
	  insert(
			  ptable[ hash ( regs.regs_PC ) ].p,
			  get_writer(INT_REGS,RA),
			  pred_PC2 == regs.regs_NPC,
			  regs.regs_NPC
		);
	  ...
	  if (pred)
	    ...
	    // both predictions are evaluated . time to decide among the two.
	    if(is_newpred_acceptable(hash(regs.regs_PC)))
	    {
		    // new pred is good to use now
		    if(f1)
		    {
			    ++no_misses;
			    f1=0;
...
//		   	    printf("new : good pred : %d  miss pred : %d \n",no_good_pred,no_miss_pred);
//		 
		    
	    }   
	    else
	    {
		    // old pred is good to use now
		    if(f2)
		    {
			    ++no_misses;
			    f2=0;
	    		...
	    		no_old++;
	//	printf("old : good pred : %d  miss pred : %d \n",no_good_pred,no_miss_pred);
	    }
	    // update the tournament predictor
      
	    if(pred_PC2 == regs.regs_NPC && pred_PC == regs.regs_NPC)
      	    {
		// both pred doing well
      	    }
      	    else if(pred_PC2 == regs.regs_NPC && pred_PC != regs.regs_NPC)
      	    {
	    	// new pred doing well
		inc_confidence(hash(regs.regs_PC));
      	    }
	    else if(pred_PC2 != regs.regs_NPC && pred_PC == regs.regs_NPC)
      	    {
	    	// old pred doing well
	      	dec_confidence(hash(regs.regs_PC));
      	    }
      	    else
      	    {
	    	// both doing equally bad
      	    }
